...
namespace Infra.Repositorio.RepositorioDespesa
{
            ...
            using (var banco = new ContextBase(_OptionsBuilder))
            {
                return await
                    (from sf in banco.SistemaFinanceiro
                     ...
                     where us.EmailUsuario.Equals(emailUsuario) && sf.Mes == d.Mes && sf.Ano == d.Ano
                     select d).AsNoTracking().ToListAsync();
            }
        }
        public async Task<IList<Despesa>> ListarDespesaNaoPagaMesesAnterior(string emailUsuario)
        {
            using (var banco = new ContextBase(_OptionsBuilder))
            {
                return await
